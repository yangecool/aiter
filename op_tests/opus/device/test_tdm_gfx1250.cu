 ...
 * @brief Device test for opus::tdm_window with 2-slot LDS ping-pong on gfx1250.
 *
 * Kernel: 32x64xK WMMA GEMM driven by tdm_window K-step.
 *   - 2-WG cluster (cluster_dims(2,1,1)); A multicast via SelectedWgs=<0,1>.
 *   - 4 producer waves load A/B tiles to LDS using tdm_window::move() over K.
...
void tdm_gfx1250_kernel(const void* __restrict__ ptr_a,
                          ...
                          int stride_c)
{
    ...
    constexpr int consumerSubWarpNum   = 4;
    DECLARE_NAMED_BARRIERS();

    const int      wave_id                = static_cast<int>(waveid_in_workgroup());
    const int      sub_consumer_wave_id   = wave_id % 4;
    const unsigned int cluster_workgroup_id_x = __builtin_amdgcn_cluster_workgroup_id_x();
    const int  c_cluster_offset_elems = static_cast<int>(cluster_workgroup_id_x) * Block_N;
    ...
    if (wave_id < 4) {
        if (wave_id < 2) {
            ...
            win_a.make(smembase, ptr_a, wave_lds_off,
                       ...
                       static_cast<unsigned long long>(stride_a),
                       0, static_cast<unsigned int>(wave_id * 16));
            ...
            win_b.make(smembase + slot_bytes_A, ptr_b, wave_lds_off,
                       ...
                       static_cast<unsigned int>(Block_N - (wave_id - 2) * 16),
                       static_cast<unsigned long long>(stride_b),
                       0, b_origin1);
    ...
    // ────────────────────────────── consumers ──────────────────────────────
        ...
        constexpr int AKSldPack    = 16 / static_cast<int>(sizeof(fp16_t));
        constexpr int AKSldLane    = 16 / AKSldPack;
        constexpr int AMSldLane    = get_warp_size() / AKSldLane;
        constexpr int AMSldRepeat  = Block_M / (AMSldLane * consumerSubWarpNum / 2);
        constexpr int AKSldRepeat  = Block_K / (AKSldPack * AKSldLane);
        static_assert(AKSldLane * AMSldLane == get_warp_size(), "A sld lane product");
        constexpr int SMemKPitch   = Block_K + 8;
        ...
        constexpr int BSldKPack   = 16 / static_cast<int>(sizeof(fp16_t));
        constexpr int BSldKLane   = 16 / BSldKPack;
        constexpr int BSldNLane   = get_warp_size() / BSldKLane;
        constexpr int BSldNRepeat = Block_N / (BSldNLane * consumerSubWarpNum / 2);
        constexpr int BSldKRepeat = Block_K / (BSldKPack * BSldKLane);
        ...
        const int sub_consumer_wave_m = sub_consumer_wave_id / 2;
        const int sub_consumer_wave_n = sub_consumer_wave_id % 2;
        const int lid                 = static_cast<int>(lane_id());
        const int a_lane_m            = lid / AMSldLane;
        const int a_lane_n            = lid % AMSldLane;
        const int b_lane_m            = lid / BSldNLane;
        const int b_lane_n            = lid % BSldNLane;
        ...
        for (int ks = 0; ks < K_STEPS; ++ks) {
            ...
            const int slot_offset = (ks & 1) * slot_pair_bytes;
            ...
            for (int kt = 0; kt < K_WmmaTiles; ++kt) {
                ...
                const int a_sld_os0 = slot_offset
                    + block_sld_win_a(0_I, sub_consumer_wave_m, kr0, a_lane_m, a_lane_n, 0_I) * static_cast<int>(sizeof(fp16_t));
                const int a_sld_os1 = slot_offset
                    + block_sld_win_a(0_I, sub_consumer_wave_m, kr1, a_lane_m, a_lane_n, 0_I) * static_cast<int>(sizeof(fp16_t));
                const int b_sld_os0 = slot_offset + slot_bytes_A
                    + block_sld_win_b(0_I, sub_consumer_wave_n, kr0, b_lane_m, b_lane_n, 0_I) * static_cast<int>(sizeof(fp16_t));
                const int b_sld_os1 = slot_offset + slot_bytes_A
                    + block_sld_win_b(0_I, sub_consumer_wave_n, kr1, b_lane_m, b_lane_n, 0_I) * static_cast<int>(sizeof(fp16_t));
        ...
        constexpr int CGstNPack = 8;
        constexpr int CGstNLane = 2;
        constexpr int CGstMLane = 16;
        ...
        int c_offset_elem = block_gmem_gst_win_c(sub_consumer_wave_m, sub_consumer_wave_n, lid / 16, lid % 16, 0_I) + c_cluster_offset_elems;
        *(reinterpret_cast<fp16x8_t*>(reinterpret_cast<fp16_t*>(ptr_c) + c_offset_elem)) = v_c;
    }
}

#else  // !__gfx1250__
__global__ void tdm_gfx1250_kernel(const void*, const void*, void*, int, int, int) {}
#endif // __gfx1250__
...
extern "C" void run_tdm_gfx1250(const void* d_a, const void* d_b, void* d_c,
                                  int stride_a, int stride_b, int stride_c)
{
    // 2 WGs forming a cluster (cluster_dims(2,1,1)); each WG has 256 threads = 8 waves.
    hipLaunchKernelGGL(tdm_gfx1250_kernel,
